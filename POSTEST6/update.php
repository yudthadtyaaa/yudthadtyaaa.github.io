...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $stmt = $pdo->prepare("UPDATE konser SET id = ?, nama = ?, email = ?, nohp = ?, tanggaldanwaktu = ?, jumlahtiket = ?, jenistiket = ?, buktibayar = ? WHERE id = ?");
        $stmt->execute([$id, $nama, $email, $nohp, $tanggaldanwaktu, $jumlahtiket, $jenistiket, $buktibayar, $_GET['id']]);
